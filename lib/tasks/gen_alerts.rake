namespace :db do
  ...
  task :alerts, [:lat, :long] => [:environment, "db:update"] do |task, args|
    ...
    old_reports = WeatherReport.where(:date => old_dates, :latitude => args.lat, :longitude => args.long)
    print "Found #{old_reports.count} old reports "
    old_reports.each {|r| old_sum += r.precip}
    puts " for #{old_sum / 10} mm of precipitation"
    recent_reports = WeatherReport.where(:date => recent_dates, :latitude => args.lat, :longitude => args.long)
